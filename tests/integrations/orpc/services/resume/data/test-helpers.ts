 ...
 * The helpers centralize database setup/teardown and provide reusable data fixtures
...
import { type DraftData, draftFactory } from "@/schema/resume/data";
 ...
 * creates the minimal schema required by the data service.
 *
 * @returns The data service module bound to the in-memory database.
 */
export const setupDraftServiceTestContext = async () => {
	...
	return await import("@/integrations/orpc/services/resume/data");
};

/**
 * @remarks
 * Removes all data rows from the in-memory database to keep test isolation.
 */
