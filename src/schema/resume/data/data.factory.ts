 ...
 *
 * Design goals:
 * - Single source of truth for data defaults.
 * - Stable, typed entry points for list item and section construction.
 * - Ergonomic nesting that mirrors the ResumeData shape.
 *
 * @see {@link ./data.schema | DraftResume schema}
 * @see {@link ./data.types | DraftResume types}
 */
import type { ResumeData, DraftResume, LabeledURL } from "./data.types";
 ...
 */
const sectionItemFactories: { [K in SectionKey]: (id: string) => ResumeData["sections"][K]["items"][number] } = {
 ...
 */
const sectionFactories: { [K in SectionKey]: () => ResumeData["sections"][K] } = {
 ...
 * @example { title: "", items: [] }
 */
const createEmptySection = <K extends SectionKey>(section: K): ResumeData["sections"][K] => sectionFactories[section]();
...
const createEmptySectionItem = <K extends SectionKey>(
	section: K,
	id: string,
): ResumeData["sections"][K]["items"][number] => sectionItemFactories[section](id);
...
/**
 * @remarks Creates the canonical empty data payload.
 * @returns A ResumeData payload populated with empty values.
 ...
 */
const createEmptyDraft = (): ResumeData => ({
